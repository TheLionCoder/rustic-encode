...
use crate::data_loading::{is_hidden, is_text_or_csv_file, read_file, extract_file_stem};
...
pub(crate) fn process_files() -> Result<(), Box<dyn std::error::Error>> {
    ...
    WalkDir::new(data_dir)
        ...
        .for_each(|entry| {
            ...
            let writers: Arc<Mutex<HashMap<String, BufWriter<File>>>> = Arc::new(Mutex::new(HashMap::new()));

            let data: Vec<u8> = read_file(file_path).unwrap();
            let decoded_data: Vec<String> = decode_data(&data);
            write_records_to_csv(decoded_data, file_stem, writers).unwrap()
        });
...
fn write_records_to_csv(decoded_data: Vec<String>, file_stem: &str,
writers: Arc<Mutex<HashMap<String, BufWriter<File>>>> ) -> Result<(), csv::Error> {
    let mut lock_writers: MutexGuard<HashMap<String, BufWriter<File>>> = writers.lock().unwrap();
    let writer: &mut BufWriter<File> = lock_writers.entry(file_stem.to_string()).or_insert_with(|| {
